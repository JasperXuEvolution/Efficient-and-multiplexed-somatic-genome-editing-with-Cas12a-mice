...
def main():
    ...
    parser.add_argument("--a1", required=True, help="This is the input fastq gz file r1")
    parser.add_argument("--a2", required=True, help="This is the input fastq gz file r2")
    ...
    temp_pattern1 = regex.compile('TAGTT' + '(.{16})' + 'TATGG' + DR1 + '(.{23})' + DR2[:10])
    temp_pattern2 = regex.compile(DR3[10:] + '(.{23})' + DR6+'TTTT')
    ...
    temp_gRNA1_list, temp_gRNA3_list, temp_read_ID_list, temp_Clonal_barcode_list, temp_label_list = [], [], [], [], []
        ...
        while temp_readID:
            ...
            if temp_search_result1:
                if temp_search_result2:
                    ...
                    temp_gRNA3 = temp_search_result2.group(1)
                    ...
                    if (temp_gRNA1 in ref_sgRNA_df.Guide1_sequence.values)&(temp_gRNA3 in ref_sgRNA_df.Guide3_sequence.values):
                        temp_matched_read+=1
    ...
    Final_df = pd.DataFrame({'gRNA1': temp_gRNA1_list, 'gRNA3': temp_gRNA3_list,
                                   'Clonal_barcode':temp_Clonal_barcode_list, 'Read_ID': temp_read_ID_list,
                                   'Sample_ID':temp_sample_ID,'Class':temp_label_list})
    Final_df['gRNA_combination'] = Final_df['gRNA1']+'_'+Final_df['gRNA3']

    temp_address_file = output_dir+'/Unexpected_reads.csv'
    temp_df = Final_df[Final_df.Class=='Unexpected']
    temp_df.to_csv(temp_address_file, index=False)
    # Output intermediate files
    Final_df = Final_df[Final_df.Class!='Unexpected']
    Final_df.to_csv(output_dir + '/Intermediate_df.csv', index=False)
    sgRNA_groups = Final_df.groupby("gRNA_combination")
